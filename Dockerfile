...
RUN apk add curl
RUN curl https://get.helm.sh/helm-v3.2.4-linux-amd64.tar.gz | tar xz
RUN curl -L -o /usr/bin/kubectl https://storage.googleapis.com/kubernetes-release/release/v1.18.5/bin/linux/amd64/kubectl
RUN curl -L -o /usr/bin/helmfile https://github.com/roboll/helmfile/releases/download/v0.119.1/helmfile_linux_amd64
RUN curl -L https://github.com/kubernetes-sigs/kustomize/releases/download/kustomize%2Fv3.8.1/kustomize_v3.8.1_linux_amd64.tar.gz | tar xz
